...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) { // Se houver query string
      ...
      try {
         ...
         const lista = await Veiculo.find().populate(
            // 1º nível
            {path:'codUsuario', select:'userName', populate:{
               // 2º nível
...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Veiculo.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Veiculo.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Veiculo.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Veiculo.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)

...
controller.novo = async (req, res) => {
   try {
      await Proposta.create(req.body)
...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) { // Se houver query string
      ...
      try {
         ...
         const lista = await Proposta.find().populate(
            //1º Nível comprador
            {path: 'idComprador', select:'userName endereco', populate:
               //2º Nível comprador
...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Proposta.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Proposta.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Proposta.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Proposta.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
